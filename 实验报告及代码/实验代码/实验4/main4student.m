function main()
	...
	stage=[10:10]*trainIter/10;	%把训练分成多个连续的阶段，将会在每个训练阶段结束之后进行一次测试，（以便观察测试error是否比上个阶段降低）
	%
	%%%%%%%%%%%%%%%
...
function [net,Verror]=netTrain(net,input,target)
	...
	if strcmp(net.hidfn,'sigmoid')==1
		dk = hidState.*(1-hidState).*(net.wout'*dm);
	else
